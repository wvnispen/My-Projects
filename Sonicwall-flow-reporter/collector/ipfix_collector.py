...
from sonicwall_templates import (
    get_field_definition, get_protocol_name, 
    decode_ipv4, decode_uint16, decode_uint32
)
...
class IPFIXCollector:
    ...
    def _parse_flow_record(self, data: bytes, template: List[Tuple[int, int, Optional[int]]], 
        ...
        for field_id, field_length, enterprise_id in template:
            ...
            if field_def:
                ...
                # Store unknown fields with generic name
                flow[f'field_{field_id}_{enterprise_id or 0}'] = field_data.hex()
    ...
    def _enrich_flow(self, flow: Dict[str, Any]):
        ...
        packets_in = flow.get('packets_in', 0)
        packets_out = flow.get('packets_out', 0)
        flow['packets_total'] = packets_in + packets_out
        
        # Calculate flow duration
        if 'flow_start' in flow and 'flow_end' in flow:
            duration = flow['flow_end'] - flow['flow_start']
        ...
        if 'src_ip' in flow:
            ...
            if identity:
                ...
                flow['user_department'] = identity.get('department')
                flow['user_location'] = identity.get('location')
